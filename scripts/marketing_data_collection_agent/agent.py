...
async def _call_mcp_tool(tool_name: str, arguments: dict) -> dict:
    ...
    headers = {"Authorization": f"Bearer {identity_token}"} if identity_token else {}
...
# ─────────────────────────────────────────────
# 1) BIGQUERY FETCH FUNCTIONS (With Debug Error Catching)
# ─────────────────────────────────────────────
...
def process_marketing_and_sf_data(
    ...
    # 3. Build Total Performance Summary Overall
    campaign_aggregates = {}
    ...
    # 4. Build Monthwise Performance Trend
    monthly_aggregates = {}
    for ad in all_ad_rows:

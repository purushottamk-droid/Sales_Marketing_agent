...
#          "<b>Lead-to-Opportunity Rate:</b>", followed by the
#          lead_to_opportunity_rate value reproduced EXACTLY as given in
#          CHANNEL_SUMMARY — do not round, reformat, reword, or alter it
#          in any way.
#        - Quality Assessment: bold label "<b>Quality Assessment:</b>",
#          followed by the quality_assessment value reproduced EXACTLY as
#          given in CHANNEL_SUMMARY, word-for-word, in full — do not
#          summarize, shorten, paraphrase, or replace it with a single
#          word.
#        Every value in this section must be copied as-is from
#        CHANNEL_SUMMARY, with zero rewording. Follow this exact field
#        order and styling for every channel entry — do not reorder or
#        omit fields for any channel.
...
def ACTION_PROMPT(ctx) -> str:
    ...
    return f"""
You are the email-notification step of the Marketing Growth Agent
pipeline. Your job: decide whether to send two emails and call the
right tools. The email HTML is built automatically by the tools — you
do NOT write any HTML, and you do NOT write any Python code. You only
pass the data through as-is.

MANAGER_EMAIL (fixed value — use exactly, never invent): {manager_email}
REPORT_EMAIL (fixed value — use exactly, never invent): {report_email}

(no data blocks needed — notify_manager and notify_report now read
campaigns_manager, channel_summary, recommended_next_steps,
executive_summary, and campaigns_full directly from session state
internally.)
...
Otherwise call notify_manager ONCE with:
  - manager_email: MANAGER_EMAIL above (never invent)
  (no other arguments — the tool builds the email content internally
  from session state)
...
If there are no campaigns for this run: record ONE SKIPPED entry with
reason "no campaigns in this run", do not call notify_report.
...
- If a tool returns status "ERROR", reflect that accurately — do not
  silently retry.
- Never invent manager_email or report_email.
- Never write HTML. Your only job is to decide skip vs. send and call
  notify_manager/notify_report with just the recipient email — the
  tools read all content directly from session state internally.
